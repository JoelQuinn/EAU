...
# read files generated by SoupX corrections
hgx <- Read10X("path/to/SoupX_Healthy")
eau1gx <- Read10X("path/to/SoupX_EAU1")
eau2gx <- Read10X("path/to/SoupX_EAU2")

# Healthy Seurat object creatiom
h <- CreateSeuratObject(hgx, project="Healthy", min.cells = 3, min.features = 200)
# EAU 1 Seurat object creatiom
e1 <- CreateSeuratObject(eau1gx, project="EAU_1", min.cells = 3, min.features = 200)
# EAU2 Seurat object creatiom
...
# Subset samples based on QC metrics
h <- subset(h, 
...
# MG-rod doublet removal (common contaminant in this cluster)
...
# Look at metadata to get DF-generated column title
eau1[[]]
...
# Trialling different parameter values. Went with nExp of 3% and pK of 0.005.
